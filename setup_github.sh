...
OWNER="${GITHUB_OWNER:-RayTrunk}"
REPO="${REPO_NAME:-faromail}"
VISIBILITY="${VISIBILITY:-public}"
DEFAULT_BRANCH="${DEFAULT_BRANCH:-master}"
DESCRIPTION="${REPO_DESCRIPTION:-FARO Mail — lightweight local-first multi-account email client}"
...
[[ -f neutralino.config.json && -d resources && -d engine ]] || {
  echo "Ce script doit être placé à la racine de FARO Mail." >&2
  exit 1
}
...
text = re.sub(r"github\.com/[^/\s)]+/faromail", f"github.com/{sys.argv[1]}/{sys.argv[2]}", text)
...
if git diff --cached --quiet; then
  ...
  git commit -m "Initial publication of FARO Mail $(cat VERSION)"
fi
