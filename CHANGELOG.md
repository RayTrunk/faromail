...
- Un fichier PDF déposé n'est plus ouvert directement dans FARO Mail.
...
# FARO Mail 0.4.2 - 2026-08-21

FARO Mail 0.4.2 améliore la gestion de la relève du courrier et ajoute une nouvelle fonction d'import de messages au format EML.

## Nouveautés

### Import de messages EML

FARO Mail peut désormais importer un ou plusieurs messages `.eml` provenant d'un autre logiciel de messagerie ou d'une archive existante.
...
- Gestion normale des messages importés dans FARO Mail.

Les messages importés restent entièrement locaux : aucune copie n'est envoyée vers le serveur IMAP ou POP3.

Ils sont stockés dans le magasin chiffré de FARO Mail et restent isolés de la synchronisation avec le serveur de messagerie.
...
- réduction de l'impact d'une relève lente ou bloquée sur les autres opérations de FARO Mail.

## Stockage et sécurité

Les messages EML importés utilisent le même stockage local chiffré que les messages récupérés normalement par FARO Mail.
...
- Les nouvelles sauvegardes FARO Mail excluent les mots de passe des comptes.
...
### Windows
- Suppression du lanceur `FARO Mail.vbs` et des lanceurs PowerShell/CMD distribués.
- Lancement direct de FARO Mail via `FARO Mail.exe`.
...
- La couleur choisie dans FARO Mail devient prioritaire sur celle éventuellement fournie par le calendrier distant.
...
- Ajout d’un planning local intégré à FARO Mail.
...
- Stockage des rendez-vous dans SQLite : ils sont inclus dans la sauvegarde complète FARO Mail.
...
- Ajout d’une fenêtre À propos simplifiée : auteur R.Trunk / FARO Solutions, licence Apache 2.0, stockage local, moteur intégré.
...
All notable changes to FARO Mail are documented here.
