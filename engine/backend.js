/**
 * FARO Mail — Moteur Node.js portable
 * API RPC JSON locale sur WebSocket (127.0.0.1:47800).
 */
'use strict';
const path = require('path');
// Met en cache le bytecode V8 compilé des modules (ws, mailparser, ./lib/*…)
// dans data/.compile-cache : le premier démarrage compile normalement, mais
// chaque démarrage suivant réutilise le cache et évite de reparser/recompiler
// des dizaines de fichiers JS — un gain sensible sur un disque lent ou sous
// antivirus temps réel, où c'est justement le coût dominant du démarrage.
try {
...
const tnef = require('./lib/tnef');
const contactSubscriptions = require('./lib/contact_subscriptions');
const vcard = require('./lib/vcard');
const certTrust = require('./lib/cert_trust');
...
const RESTORE_STATE_FILE = path.join(ROOT, '.faromail-restore-state.json');
const RETENTION_CHECK_MS = 6 * 60 * 60 * 1000;
const OUTBOX_CHECK_MS = 30 * 1000;
const CALENDAR_SUBSCRIPTION_CHECK_MS = 60 * 1000;
const CONTACT_SUBSCRIPTION_CHECK_MS = 60 * 1000;
...
function recoverInterruptedRestore() {
  ...
  if (safeRollback && fs.existsSync(rollbackRoot)) {
    try {
      ...
      console.warn('[FARO Mail] Une restauration interrompue a été annulée automatiquement.');
    } catch (error) {
      console.error('[FARO Mail] Impossible d’annuler la restauration interrompue :', error.message);
      throw error;
    }
  }
...
const defaultConfig = {
  ...
  listPaneHeight: 330,
  trustedSenders: [],
  // Domaines pour lesquels les contenus distants (images, styles, etc.) sont
  // toujours affichés, quel que soit le réglage blockRemoteImages.
  remoteContentWhitelist: [],
};
...
function normalizeAliasEntry(alias) {
  const email = String(alias?.email || '').trim().toLowerCase();
  if (!EMAIL_PATTERN.test(email)) return null;
  return {
    id: String(alias?.id || '').trim() || crypto.randomUUID(),
    name: String(alias?.name || '').trim().slice(0, 120),
    email: email.slice(0, 200),
    replyTo: (() => {
      const replyTo = String(alias?.replyTo || '').trim().toLowerCase();
      return replyTo && EMAIL_PATTERN.test(replyTo) ? replyTo.slice(0, 200) : '';
    })(),
  };
}
...
function normalizeAccount(account) {
  ...
  return {
    ...
    // Dossiers IMAP supplémentaires choisis manuellement par l'utilisateur,
    // synchronisés et affichés en plus des quatre dossiers spéciaux détectés
    // automatiquement (boîte de réception, envoyés, corbeille, indésirables).
    extraFolders: receiveProtocol === 'pop3'
      ? []
      : (Array.isArray(account.extraFolders)
          ? [...new Set(account.extraFolders.map(folder => String(folder || '').trim()).filter(Boolean))].slice(0, 50)
          : []),
    // Identités supplémentaires (« envoyer en tant que ») partageant les
    // identifiants SMTP du compte, mais avec une adresse/un nom d'expéditeur
    // différent. Ne concerne que l'en-tête From du message envoyé.
    aliases: Array.isArray(account.aliases)
      ? account.aliases.map(normalizeAliasEntry).filter(Boolean).slice(0, 20)
      : [],
  };
}
...
async function runSecureSearchMigration() {
  ...
  if (!storage.available) {
    console.warn(`[FARO Mail] Index de recherche chiffré indisponible : ${storage.error || 'coffre-fort système indisponible'}`);
  ...
  console.log(`[FARO Mail] Sécurisation de l’index local : ${rows.length} message(s) à traiter...`);
  ...
  for (const row of rows) {
    try {
      ...
      const parsed = await tnef.expandTnefAttachments(await simpleParser(raw, { skipImageLinks: true }));
      ...
      console.warn(`[FARO Mail] Index sécurisé : message ${row.id} ignoré : ${error.message}`);
    }
    if (indexed && indexed % 250 === 0) {
      console.log(`[FARO Mail] Index sécurisé : ${indexed}/${rows.length} message(s)`);
    }
  }
  ...
  console.log(`[FARO Mail] Index local sécurisé : ${indexed} message(s), ${failed} erreur(s).`);
...
function loadRuntimeState() {
  ...
  accounts = rawAccounts.map(account => {
    try {
      ...
      console.warn(`[FARO Mail] Compte ${account.email || account.id} : ${error.message}. Le mot de passe existant n'a pas été supprimé.`);
      return account;
    }
  });
  if (migrated) {
    saveAccounts();
    console.log('[FARO Mail] Les mots de passe existants ont été migrés vers le coffre-fort système.');
  }
...
// Le serveur WebSocket écoute uniquement sur 127.0.0.1, mais la politique de
// même origine ne s'applique pas aux handshakes ws:// : n'importe quel onglet
// de navigateur ouvert localement pourrait sinon s'y connecter et appeler
// l'intégralité de l'API RPC (lecture des messages, envoi de courrier,
// suppression de comptes…). Un jeton généré à chaque démarrage, écrit dans un
// fichier lu uniquement par l'interface Neutralino, sert de secret partagé.
const RPC_TOKEN = crypto.randomBytes(32).toString('hex');
try {
  fs.writeFileSync(path.join(DATA, 'engine.token'), RPC_TOKEN, { mode: 0o600 });
} catch (error) {
  console.error('[FARO Mail] Impossible d’écrire le jeton du moteur :', error.message);
}

// Certaines méthodes RPC (export/import de sauvegarde, import EML) acceptent
// un chemin de fichier arbitraire. Le sélecteur natif (nativeDialog) tourne
// côté moteur : on retient donc les chemins qu'il vient de renvoyer et on
// n'accepte ensuite que ceux-là, plutôt que de faire confiance à n'importe
// quel chemin fourni par l'appelant RPC.
const GRANTED_PATH_TTL_MS = 15 * 60 * 1000;
const grantedPaths = new Map(); // chemin absolu -> { purpose, expiresAt }

function grantPaths(paths, purpose) {
  const expiresAt = Date.now() + GRANTED_PATH_TTL_MS;
  for (const raw of Array.isArray(paths) ? paths : [paths]) {
    if (!raw) continue;
    grantedPaths.set(path.resolve(String(raw)), { purpose, expiresAt });
  }
}

function consumeGrantedPath(rawPath, purpose) {
  const resolved = path.resolve(String(rawPath || ''));
  const grant = grantedPaths.get(resolved);
  grantedPaths.delete(resolved);
  if (!grant || grant.purpose !== purpose || grant.expiresAt < Date.now()) return null;
  return resolved;
}

function isInsideAppInternals(resolvedPath) {
  const internalRoots = [DATA, path.join(ROOT, 'engine')];
  return internalRoots.some(root => resolvedPath === root || resolvedPath.startsWith(root + path.sep));
}

function isAuthorizedRequest(request) {
  let token = '';
  try {
    token = new URL(request.url || '/', 'http://127.0.0.1').searchParams.get('token') || '';
  } catch { return false; }
  const provided = Buffer.from(token, 'utf8');
  const expected = Buffer.from(RPC_TOKEN, 'utf8');
...
let contactSubscriptionTimer = null;
let contactSubscriptionSyncBusy = false;
  ...
  const mailMigration = await mailStore.migrateLegacyMessages(db.db, {
    onProgress: progress => {
      if (progress.encrypted === 1 || progress.encrypted % 250 === 0) {
        console.log(`[FARO Mail] Migration du stockage local : ${progress.encrypted}/${progress.total} message(s) chiffré(s)`);
        if (notify) broadcast('storage.migration.progress', { phase: 'messages', ...progress });
      }
    },
  });
  if (mailMigration.encrypted) {
    console.log(`[FARO Mail] Stockage local : ${mailMigration.encrypted} ancien(s) .eml migré(s) vers le magasin chiffré.`);
  }
...
wss.on('listening', () => {
  serverReady = true;
  console.log(`[FARO Mail ${APP_VERSION}] Moteur prêt sur ws://127.0.0.1:${PORT} — ${accounts.length} compte(s)`);
  ...
  new Promise(resolve => setImmediate(resolve))
    ...
    .catch(error => {
      console.error('[FARO Mail] Migration du stockage 0.4.0 impossible :', error);
...
wss.on('error', error => {
  console.error('[FARO Mail] Impossible de démarrer le serveur local :', error);
...
function publicAccount(account) {
  return {
    ...
    extraFolders: Array.isArray(account.extraFolders) ? account.extraFolders : [],
...
/**
 * Vérifie une connexion SMTP/IMAP/POP3 candidate et transforme une erreur
 * réseau/TLS brute (souvent en anglais, venant des bibliothèques sous-jacentes)
 * en erreur structurée : { code, protocol, certDetails? }. Le frontend traduit
 * ensuite ce code dans la langue de l'utilisateur ; certDetails alimente la
 * boîte de dialogue de confiance de certificat.
 */
async function verifyProtocolConnection(protocol, action, account) {
  try {
    return await action();
  } catch (error) {
    const code = certTrust.classifyConnectionError(error);
    const prefixed = new Error(`${protocol.toUpperCase()} : ${error.message}`);
    if (code === 'CERT_UNTRUSTED') {
      const target = protocol === 'smtp' ? account.smtp : (protocol === 'pop3' ? account.pop3 : account.imap);
      let certDetails = null;
      try { certDetails = await certTrust.probeCertificate(target?.host, target?.port); } catch {}
      throw Object.assign(prefixed, { code, protocol, certDetails });
    }
    throw Object.assign(prefixed, { code, protocol });
  }
}

function verifySmtpConnection(account) {
  return verifyProtocolConnection('smtp', () => smtp.verify(account), account);
}

function verifyIncomingConnection(receiveProtocol, account) {
  if (receiveProtocol === 'pop3') return verifyProtocolConnection('pop3', () => pop3.verify(account), account);
  return verifyProtocolConnection('imap', () => imap.resolveFolderMap(account), account);
}

/**
 * Détermine l'empreinte de certificat épinglée à conserver lors d'une mise à
 * jour de compte : une nouvelle empreinte explicitement soumise l'emporte,
 * sinon l'ancienne n'est conservée que si l'hôte/port n'a pas changé (un
 * changement de serveur doit redemander confirmation).
 */
function carriedCertFingerprint(inputProtocol, currentProtocol, nextHost, nextPort) {
  const submitted = String(inputProtocol?.trustedCertFingerprint || '').trim();
  if (submitted) return submitted;
  const sameTarget = currentProtocol
    && String(currentProtocol.host || '') === nextHost
    && (Number(currentProtocol.port) || 0) === nextPort;
...
function forceCancelSync(accountId, runId) {
  ...
  console.warn(`[FARO Mail] Relève ${accountId} libérée par le watchdog après ${SYNC_CANCEL_WATCHDOG_MS} ms.`);
  return true;
}
...
function cancelActiveSyncs(accountId = null) {
  ...
  // ci-dessus garantit en plus que l'état FARO Mail est libéré même si une
    ...
    try {
      if (account.receiveProtocol === 'pop3') {
        ...
        if (result.addedIds?.length) {
          await applyMailRulesForNewMessages(account, 'INBOX', result.addedIds).catch(error =>
            console.warn('[FARO Mail] Règles de messagerie (POP3) :', error.message));
        }
      ...
      const specialFolders = new Set([folders.inbox, folders.sent, folders.trash, folders.junk].filter(Boolean));
      const allJobs = [
        ['inbox', folders.inbox || 'INBOX'],
        ['sent', folders.sent],
        ['trash', folders.trash],
        ['junk', folders.junk],
        // Dossiers supplémentaires choisis manuellement (hors doublons avec
        // les quatre dossiers spéciaux déjà synchronisés ci-dessus).
        ...(Array.isArray(account.extraFolders) ? account.extraFolders : [])
          .filter(folder => folder && !specialFolders.has(folder))
          .map(folder => ['other', folder]),
      ...
      const inboxResult = results.find(result => result.role === 'inbox');
      const added = Number(inboxResult?.added) || 0;
      if (inboxResult?.addedIds?.length) {
        await applyMailRulesForNewMessages(account, inboxResult.folder, inboxResult.addedIds).catch(error =>
          console.warn('[FARO Mail] Règles de messagerie :', error.message));
      }
...
function roleForAccountFolder(account, folder) {
  if (folder === account.folderMap?.inbox) return 'inbox';
  if (folder === account.folderMap?.sent) return 'sent';
  if (folder === account.folderMap?.trash) return 'trash';
  if (folder === account.folderMap?.junk) return 'junk';
  return 'other';
}

/** Traduit l'action « déplacer » stockée dans une règle (mot-clé de rôle ou
 * chemin de dossier IMAP supplémentaire) en chemin de dossier réel pour ce
 * compte précis. Une règle est toujours liée à un compte : les chemins ne
 * sont jamais réutilisés d'un compte à l'autre. */
function resolveRuleMoveTarget(account, moveTo) {
  if (!moveTo || !account) return null;
  if (moveTo.type === 'role') return account.folderMap?.[moveTo.value] || null;
  if (moveTo.type === 'path') {
    const path = String(moveTo.value || '');
    const known = Array.isArray(account.extraFolders) && account.extraFolders.includes(path);
    return known ? path : null;
  }
  return null;
}

/** Applique les règles de messagerie activées d'un compte aux messages qui
 * viennent d'être ajoutés en boîte de réception (relève IMAP ou POP3). Les
 * écritures locales (lu, suivi, indésirable, étiquette) sont immédiates ; un
 * déplacement de dossier réutilise exactement le mécanisme déjà éprouvé de
 * « déplacer vers la corbeille » : commande IMAP MOVE, suppression de la
 * ligne locale, puis relève ciblée du dossier de destination pour la
 * réimporter proprement (évite toute confusion avec la réconciliation
 * incrémentale du dossier source lors de la prochaine relève).
 */
async function applyMailRulesForNewMessages(account, folder, messageIds) {
  if (!account || !Array.isArray(messageIds) || !messageIds.length) return;
  const rules = db.listMailRules(account.id).filter(rule => rule.enabled);
  ...
  for (const id of messageIds) {
    ...
    if (!row) continue;
    const ctx = {
      fromName: row.from_name, fromAddr: row.from_addr, toAddr: row.to_addr,
      subject: row.subject, bodyText: '',
    };
    const matched = mailRules.evaluate(rules, ctx);
    if (!matched.length) continue;
    const outcome = mailRules.mergeActions(matched);
    if (!mailRules.hasOutcome(outcome)) continue;

    if (outcome.markRead === true) { db.setFlag(id, 'seen', true); serverActionSafe(row, { type: 'seen' }); }
    if (outcome.flag === true) { db.setFlag(id, 'flagged', true); serverActionSafe(row, { type: 'flag' }); }
    if (outcome.isSpam !== null) db.setSpam(id, outcome.isSpam);
    for (const labelId of outcome.labelIds) {
      try { db.tagMessages([id], labelId); } catch {}
    }

    if (outcome.moveTo) {
      if (account.receiveProtocol === 'pop3') {
        const targetFolder = account.folderMap?.[outcome.moveTo.value] || null;
        if (targetFolder && targetFolder !== row.folder) {
          db.moveLocalMessages([id], targetFolder, roleForAccountFolder(account, targetFolder));
        }
      } else {
        const targetFolder = resolveRuleMoveTarget(account, outcome.moveTo);
        if (targetFolder && targetFolder !== row.folder) {
          if (!moveGroups.has(targetFolder)) moveGroups.set(targetFolder, []);
          moveGroups.get(targetFolder).push({ id, uid: row.uid });
        }
      }
    }
  }

  for (const [targetFolder, entries] of moveGroups) {
    try {
      await imap.moveUids(account, folder, entries.map(entry => entry.uid), targetFolder);
      db.deleteMessages(entries.map(entry => entry.id));
      await syncRole(account, targetFolder, roleForAccountFolder(account, targetFolder), 'rules').catch(() => {});
    } catch (error) {
      console.warn(`[FARO Mail] Règle de messagerie : déplacement vers ${targetFolder} échoué :`, error.message);
    }
  }
}
...
async function setMessageSpamState(message, isSpam, { persistSenderRule = false } = {}) {
  ...
  if (persistSenderRule && String(message.from_addr || '').trim()) {
    ...
    const automaticRulePrefix = 'FARO Mail:manual-spam:';
    // Les règles créées avant le renommage LibraMail -> FARO Mail portent
    // encore l'ancien préfixe : on continue à les reconnaître en lecture.
    const legacyAutomaticRulePrefix = 'LibraMail:manual-spam:';
    const matchAutomaticPrefix = notes => {
      const value = String(notes || '');
      if (value.startsWith(automaticRulePrefix)) return automaticRulePrefix;
      if (value.startsWith(legacyAutomaticRulePrefix)) return legacyAutomaticRulePrefix;
      return null;
    };
    ...
    if (isSpam) {
      ...
      const isExistingAutomatic = Boolean(matchAutomaticPrefix(existingRule?.notes));
      ...
      const matchedPrefix = existingRule?.action === 'block' ? matchAutomaticPrefix(existingRule.notes) : null;
      const automaticRule = matchedPrefix ? existingRule : null;
      if (automaticRule) {
        const encodedPrevious = String(automaticRule.notes).slice(matchedPrefix.length);
        let previous = null;
        try {
...
// FARO Mail 0.2.25 — restauration des messages supprimés par inadvertance.
...
// FARO Mail 0.2.25 v2 — une restauration demandée sur un message concerne
...
async function syncCalendarSubscription(id, { announce = true } = {}) {
  ...
  // Le choix effectué dans FARO Mail (couleur / compte associé) reste
...
async function syncContactSubscription(id, { announce = true } = {}) {
  const subscription = db.getContactSubscription(id);
  if (!subscription) throw new Error('Abonnement de contacts introuvable');
  if (!subscription.enabled) return { subscription, skipped: true, reason: 'disabled' };
  try {
    const remote = await contactSubscriptions.fetchVcard(subscription.url, {
      etag: subscription.etag,
      lastModified: subscription.lastModified,
    });
    if (remote.notModified) {
      const current = db.updateContactSubscriptionSync(subscription.id, {
        etag: remote.etag, lastModified: remote.lastModified,
        lastSyncAt: Date.now(), lastStatus: 'ok', lastError: '',
      });
      if (announce) broadcast('contacts.subscriptions.changed', { action: 'synced', id: subscription.id, notModified: true });
      return { subscription: current, notModified: true, created: 0, updated: 0, removed: 0, total: 0 };
    }
    const entries = vcard.parseVcard(remote.text);
    const result = db.syncContactSubscriptionEntries(subscription.id, entries);
    let current = subscription;
    if (!subscription.name) {
      const name = contactSubscriptions.displayNameFromUrl(subscription.url);
      if (name) current = db.saveContactSubscription({ ...subscription, name }, subscription.id);
    }
    current = db.updateContactSubscriptionSync(subscription.id, {
      etag: remote.etag, lastModified: remote.lastModified,
      lastSyncAt: Date.now(), lastStatus: 'ok', lastError: '',
    });
    if (announce) {
      broadcast('contacts.changed', { action: 'subscription-synced', id: subscription.id, ...result });
      broadcast('contacts.subscriptions.changed', { action: 'synced', id: subscription.id, ...result });
    }
    return { ...result, subscription: current };
  } catch (error) {
    const current = db.updateContactSubscriptionSync(subscription.id, {
      lastSyncAt: Date.now(), lastStatus: 'error', lastError: error.message,
    });
    if (announce) broadcast('contacts.subscriptions.changed', { action: 'error', id: subscription.id, error: error.message });
    error.subscription = current;
    throw error;
  }
}

function contactSubscriptionIsDue(subscription, now = Date.now()) {
  const refreshMinutes = Math.max(0, Number(subscription?.refreshMinutes) || 0);
  if (!refreshMinutes) return false;
  const lastSyncAt = Math.max(0, Number(subscription?.lastSyncAt) || 0);
  return !lastSyncAt || (now - lastSyncAt) >= refreshMinutes * 60 * 1000;
...
async function syncAllContactSubscriptions({ announce = false, dueOnly = false } = {}) {
  if (contactSubscriptionSyncBusy || maintenanceOperation || shuttingDown) return [];
  contactSubscriptionSyncBusy = true;
  const results = [];
  try {
    const now = Date.now();
    for (const subscription of db.listContactSubscriptions({ enabledOnly: true })) {
      if (dueOnly && !contactSubscriptionIsDue(subscription, now)) continue;
      try { results.push(await syncContactSubscription(subscription.id, { announce })); }
      catch (error) { results.push({ id: subscription.id, subscription, error: error.message }); }
    }
    return results;
  } finally {
    contactSubscriptionSyncBusy = false;
  }
}
...
function stopAccountRuntime() {
  ...
  if (contactSubscriptionTimer) clearInterval(contactSubscriptionTimer);
  contactSubscriptionTimer = null;
  imap.stopAllWatches?.();
}
...
function startAccountRuntime({ resolveFolders = false } = {}) {
  ...
  if (contactSubscriptionTimer) clearInterval(contactSubscriptionTimer);
  contactSubscriptionTimer = setInterval(() => syncAllContactSubscriptions({ announce: true, dueOnly: true }).catch(() => {}), CONTACT_SUBSCRIPTION_CHECK_MS);
  contactSubscriptionTimer.unref?.();
  setTimeout(() => syncAllContactSubscriptions({ announce: true, dueOnly: true }).catch(() => {}), 6000).unref?.();
}
...
async function importCompleteBackup(sourcePath, password = '') {
  ...
  const safetyBackup = path.join(BACKUPS_DIR, `FaroMail-avant-restauration-${backupTimestamp()}.zip`);
  ...
  const stagingRoot = fs.mkdtempSync(path.join(ROOT, '.faromail-restore-'));
  const rollbackRoot = path.join(ROOT, `.faromail-rollback-${process.pid}-${Date.now()}`);
...
const methods = {
  ...
  'backup.selectExportPath': async ({ defaultName } = {}) => {
    let selected = await nativeDialog.showBackupDialog({
      mode: 'save',
      defaultName,
      title: 'Enregistrer la sauvegarde complète de FARO Mail',
    });
    // Certains sélecteurs natifs (ex. Zenity) ne forcent pas l'extension même
    // avec un filtre *.zip : on la garantit ici, une seule fois, pour que le
    // chemin retourné soit exactement celui qui sera ensuite autorisé.
    if (selected && !selected.toLowerCase().endsWith('.zip')) selected += '.zip';
    if (selected) grantPaths(selected, 'backup-export');
    return { path: selected };
  },

  'backup.selectImportPath': async () => {
    const selected = await nativeDialog.showBackupDialog({
      mode: 'open',
      title: 'Choisir une sauvegarde FARO Mail',
    });
    if (selected) grantPaths(selected, 'backup-import');
    return { path: selected };
  },

  'backup.inspect': async ({ sourcePath }) => {
    const grantedSource = consumeGrantedPath(sourcePath, 'backup-import');
    if (!grantedSource) throw new Error('Chemin de sauvegarde non autorisé. Utilisez le sélecteur de fichier de FARO Mail.');
    grantPaths(grantedSource, 'backup-import');
    return publicBackupInspection(await backup.inspectArchive(grantedSource));
  },

  'backup.export': async ({ targetPath, password = '' }) => withMaintenance('export', async () => {
    const grantedTarget = consumeGrantedPath(targetPath, 'backup-export');
    if (!grantedTarget) throw new Error('Chemin de sauvegarde non autorisé. Utilisez le sélecteur de fichier de FARO Mail.');
    const result = await exportCompleteBackup(grantedTarget, { kind: 'export', step: 'archive', password });
    return {
  ...
  'backup.import': async ({ sourcePath, password = '' }) => withMaintenance('import', async () => {
    const grantedSource = consumeGrantedPath(sourcePath, 'backup-import');
    if (!grantedSource) throw new Error('Chemin de sauvegarde non autorisé. Utilisez le sélecteur de fichier de FARO Mail.');
    return importCompleteBackup(grantedSource, password);
  }),

  // ---------- Import local de messages EML ----------
  'eml.selectImportPaths': async () => {
    const paths = await nativeDialog.showEmlDialog({
      title: 'Importer des messages EML dans FARO Mail',
    });
    if (Array.isArray(paths) && paths.length) grantPaths(paths, 'eml-import');
    return { paths };
  },
  ...
  'eml.import': async ({ accountId, paths = [], mode = 'auto' } = {}) => {
    ...
    const grantedPathList = paths.map(candidate => consumeGrantedPath(candidate, 'eml-import'));
    if (!grantedPathList.length || grantedPathList.some(candidate => !candidate)) {
      throw new Error('Fichier(s) non autorisé(s). Utilisez le sélecteur de fichier de FARO Mail.');
    }
    return withEmlImportMaintenance(() => emlImport.importFiles({
      account,
      paths: grantedPathList,
      mode,
  ...
  'accounts.add': async input => {
    ...
    const account = normalizeAccount({
      ...
      imap: receiveProtocol === 'imap' ? {
        ...
        trustedCertFingerprint: String(input.imap.trustedCertFingerprint || ''),
      } : {},
      pop3: receiveProtocol === 'pop3' ? {
        ...
        trustedCertFingerprint: String(input.pop3.trustedCertFingerprint || ''),
      } : {},
      smtp: {
        ...
        pass: smtpPassword,
        trustedCertFingerprint: String(input.smtp.trustedCertFingerprint || ''),
      },
    ...
    if (receiveProtocol === 'pop3') {
      await verifyIncomingConnection('pop3', account);
      account.folderMap = { inbox: 'INBOX', sent: 'Local/Sent', trash: 'Local/Trash', junk: 'Local/Junk' };
    } else {
      account.folderMap = await verifyIncomingConnection('imap', account);
    }
  ...
  'accounts.update': async input => {
    ...
    const candidate = normalizeAccount({
      ...
      imap: receiveProtocol === 'imap' ? {
        ...
        user: String(input.imap.user || '').trim(),
        pass: incomingPassword,
        trustedCertFingerprint: carriedCertFingerprint(input.imap, current.imap,
          String(input.imap.host || '').trim(), Number(input.imap.port) || 993),
      } : {},
      pop3: receiveProtocol === 'pop3' ? {
        host: String(input.pop3.host || '').trim(),
        port: Number(input.pop3.port) || 995,
        ...
        trustedCertFingerprint: carriedCertFingerprint(input.pop3, current.pop3,
          String(input.pop3.host || '').trim(), Number(input.pop3.port) || 995),
      } : {},
      smtp: {
        host: String(input.smtp.host || '').trim(),
        port: Number(input.smtp.port) || 465,
        ...
        pass: smtpPassword,
        trustedCertFingerprint: carriedCertFingerprint(input.smtp, current.smtp,
          String(input.smtp.host || '').trim(), Number(input.smtp.port) || 465),
      },
    ...
    await verifySmtpConnection(candidate);
    if (receiveProtocol === 'pop3') {
      await verifyIncomingConnection('pop3', candidate);
      candidate.folderMap = { inbox: 'INBOX', sent: 'Local/Sent', trash: 'Local/Trash', junk: 'Local/Junk' };
    } else {
      candidate.folderMap = await verifyIncomingConnection('imap', candidate);
    }
  ...
  'accounts.setExtraFolders': async ({ id, folders }) => {
    ...
    saveAccounts();
    syncAccount(account, 'manual').catch(() => {});
    return publicAccount(account);
  },
  ...
  'sync.start': async ({ accountId }) => {
    ...
    syncAccount(account, 'manual').catch(error =>
      console.error(`[FARO Mail] Relève manuelle ${account.email || account.id} :`, error.message)
    );
  ...
  'sync.startAll': async () => {
    ...
    syncAllAccounts('manual').catch(error =>
      console.error('[FARO Mail] Relève manuelle de toutes les boîtes :', error.message)
    );
    ...
    setTimeout(() => {
      if (!shuttingDown && !maintenanceOperation) {
        syncAllAccounts('startup').catch(error =>
          console.error('[FARO Mail] Relève au démarrage :', error.message)
        );
      }
    }, 250).unref?.();
    return { started: true, accounts: accounts.length };
  ...
  // ---------- Règles de messagerie (filtres) ----------
  'rules.list': async ({ accountId } = {}) => db.listMailRules(accountId || null),
  'rules.save': async input => db.saveMailRule(input || {}),
  'rules.remove': async ({ id }) => {
    if (!db.removeMailRule(id)) throw new Error('Règle introuvable');
    return true;
  },
  ...
  'contacts.exportVcard': async ({ groupId } = {}) => {
    const contacts = db.listContacts({ groupId: groupId || null, limit: 5000 });
    return { vcard: vcard.buildVcardCollection(contacts), count: contacts.length };
  },
  ...
  // ---------- Abonnements de contacts (vCard, lecture seule) ----------
  'contacts.subscriptions.list': async () => db.listContactSubscriptions(),
  'contacts.subscriptions.add': async ({ name = '', url = '', refreshMinutes = 30 } = {}) => {
    const subscription = db.saveContactSubscription({ name, url, refreshMinutes });
    broadcast('contacts.subscriptions.changed', { action: 'created', id: subscription.id });
    try {
      return await syncContactSubscription(subscription.id, { announce: true });
    } catch (error) {
      return { subscription: db.getContactSubscription(subscription.id), error: error.message };
    }
  },
  'contacts.subscriptions.sync': async ({ id } = {}) => syncContactSubscription(id, { announce: true }),
  'contacts.subscriptions.syncAll': async () => syncAllContactSubscriptions({ announce: true }),
  'contacts.subscriptions.remove': async ({ id } = {}) => {
    const result = db.removeContactSubscription(id);
    if (result.removed) {
      broadcast('contacts.changed', { action: 'subscription-removed', id: Number(id), removed: result.removedContacts });
      broadcast('contacts.subscriptions.changed', { action: 'removed', id: Number(id) });
    }
    return result;
  },
  ...
  'calendar.import': async ({ text = '', fileName = '', accountId = '', color = '', locale = 'de' } = {}) => {
  ...
  'calendar.importAttachment': async ({ messageId, index } = {}) => {
    ...
    const parsed = calendarImport.parseCalendarImport({
      text, fileName, accountId, color: '', locale: config.locale || 'de',
    });
    const result = db.importCalendarEvents(parsed.events);
  ...
  'attachments.save': async ({ messageId, index, targetPath }) => {
    ...
    if (!path.isAbsolute(String(targetPath || '')) || isInsideAppInternals(resolvedTarget)) {
  ...
  // Contenu d'une pièce jointe encodé en base64, pour l'aperçu intégré
  // (visionneuse PDF). Une limite de taille évite de saturer la connexion
  // WebSocket locale avec un très gros fichier ; au-delà, l'utilisateur est
  // invité à l'enregistrer puis à l'ouvrir avec une application externe.
  ...
  socket.on('message', async buffer => {
    ...
    try {
      ...
      if (socket.readyState === 1) {
        const payload = { id, ok: false, error: error.message };
        if (error.code) payload.errorCode = error.code;
        if (error.protocol) payload.errorProtocol = error.protocol;
        if (error.certDetails) payload.certDetails = error.certDetails;
...
const launcherParentArgument = process.argv.find(argument => String(argument).startsWith('--faromail-parent-pid='));
const launcherParentPid = Number(launcherParentArgument?.split('=')[1] || process.env.FAROMAIL_PARENT_PID || 0);
if (Number.isInteger(launcherParentPid) && launcherParentPid > 0) {
  const parentWatch = setInterval(() => {
    try {
      ...
      if (error?.code === 'ESRCH') {
        console.warn('[FARO Mail] Lanceur Windows fermé : arrêt du moteur.');
        shutdown();
      }
    }
  }, 2000);
...
process.on('unhandledRejection', error => {
  ...
  // pour les connexions que FARO Mail vient explicitement de couper.
  if (imap.isExpectedCancellationError?.(error)) return;
  console.error('[FARO Mail] Promesse non interceptée :', error);
});
process.on('uncaughtException', error => {
  if (imap.isExpectedCancellationError?.(error)) return;
  console.error('[FARO Mail] Erreur non interceptée :', error);
});
