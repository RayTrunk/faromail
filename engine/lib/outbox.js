/**
 * FARO Mail 0.2.24 — brouillons et envois différés.
...
function normalizeState(input = {}) {
  ...
  const normalized = {
    ...
    fromIdentity: state.fromIdentity ? String(state.fromIdentity) : undefined,
    to: String(state.to || ''),
    cc: String(state.cc || ''),
...
function normalizeMail(input = {}) {
  ...
  const normalized = {
    ...
    fromIdentity: mail.fromIdentity ? String(mail.fromIdentity) : undefined,
