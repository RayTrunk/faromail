/**
 * FARO Mail 0.4.0 — réception POP3 locale.
 * POP3 est volontairement utilisé comme protocole de rapatriement: les messages
 * téléchargés restent dans FARO Mail même s'ils disparaissent ensuite du serveur.
 */
'use strict';
...
async function makeClient(account, clientFactory = null) {
  ...
  if (options.secure === false) {
    throw new Error('FARO Mail 0.4.0 exige SSL/TLS pour POP3 (port 995 recommandé).');
  }
  if (typeof clientFactory === 'function') return clientFactory(account);
  const Pop3Command = await loadPop3Command();
  const pinnedTls = certTrust.buildTlsOptions(options.trustedCertFingerprint);
  return new Pop3Command({
    ...
    tlsOptions: { servername: options.host, ...(pinnedTls || {}) },
  });
}
...
async function verify(account) {
  ...
  try {
    ...
    throw Object.assign(new Error(sanitizeError(error, account.pop3?.pass)), {
