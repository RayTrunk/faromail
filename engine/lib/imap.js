/**
 * FARO Mail — Synchronisation et actions IMAP (imapflow)
 * Indexation incrémentale par UID. Le message MIME brut est conservé dans
 * le magasin local chiffré depuis FARO Mail 0.4.0 (compatibilité .eml assurée).
 */
'use strict';
  ...
  setImmediate(() => {
    try {
      ...
      if (!isExpectedCancellationError(error)) {
        console.warn('[FARO Mail] Fermeture IMAP interrompue :', error?.message || error);
      }
    }
  });
  return true;
  ...
  const logoutPromise = Promise.resolve()
    ...
    .catch(error => {
      if (!isExpectedCancellationError(error) && error?.code !== 'NoConnection') {
        console.warn('[FARO Mail] Fermeture IMAP :', error?.message || error);
      }
    })
...
function makeClient(account) {
  const tlsOptions = certTrust.buildTlsOptions(account.imap?.trustedCertFingerprint);
  return new ImapFlow({
  ...
  try {
    ...
    if (source === 'manual' || totalMs >= 1500) {
      ...
      const detail = folderTimings
        ...
        .join(' · ');
      console.log(
        `[FARO Mail][IMAP] ${label} · ${source} · connexion ${seconds(connectMs)}`
        + (detail ? ` · ${detail}` : '')
