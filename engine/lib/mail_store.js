...
function readMessage(row) {
  ...
  if (String(row.storage_kind || '') === 'encrypted') {
    try {
      ...
      if (legacy) {
        console.warn(`[FARO Mail] Magasin chiffré indisponible pour le message ${row.id}; repli temporaire sur ${legacy.path}: ${error.message}`);
        return legacy.raw;
      }
      throw error;
    }
...
async function migrateLegacyMessages(indexDb, { onProgress = null } = {}) {
  if (!ensureMasterKey()) {
    console.warn(`[FARO Mail] Stockage chiffré indisponible, conservation temporaire des .eml : ${secureError}`);
