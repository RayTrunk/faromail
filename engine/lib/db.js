/**
 * FARO Mail — Index SQLite (better-sqlite3, WAL, FTS5)
...
function migrateEmlPaths(dataDir) {
    ...
    for (const row of items) {
      ...
      if (storedPath !== expected && fs.existsSync(storedPath)) {
        try {
          ...
          console.warn(`[FARO Mail] Copie impossible pour le message ${row.id}: ${error.message}`);
        }
      }
  ...
  if (repaired || copied || missing) {
    console.log(`[FARO Mail] Chemins des messages : ${repaired} rebasés; ${copied} copiés; ${missing} introuvables`);
  }
...
function init(dataDir) {
  ...
  -- Règles de messagerie (filtres) appliquées au courrier entrant : une
  -- règle est toujours rattachée à un compte précis (les dossiers cibles
  -- d'une action « déplacer » ne sont pas portables d'un compte à l'autre).
  ...
  CREATE INDEX IF NOT EXISTS idx_mail_rules_account ON mail_rules(account_id, enabled, position);
  ...
  CREATE INDEX IF NOT EXISTS idx_contact_subscriptions_enabled ON contact_subscriptions(enabled, last_sync_at);
  ...
  ensureColumn('contacts', 'import_key', 'TEXT');
  ensureColumn('contacts', 'subscription_id', 'INTEGER');
...
function listConversations({ limit = 200, offset = 0, sortBy = 'date', sortDirection = 'desc', ...filters } = {}) {
  ...
  return revealSnippets(db.prepare(`
    WITH filtered AS (
      ...
      SELECT filtered.*,
             /* FARO Mail 0.2.24 UI v16 — un dossier choisit les fils à afficher,
...
const setSpam = (id, value) => db.prepare('UPDATE messages SET is_spam=? WHERE id=?').run(value ? 1 : 0, id);
...
function listMailRules(accountId = null) {
  const rows = accountId
    ? db.prepare('SELECT * FROM mail_rules WHERE account_id=? ORDER BY position, id').all(String(accountId))
    : db.prepare('SELECT * FROM mail_rules ORDER BY account_id, position, id').all();
  return rows.map(row => ({
    ...row,
    enabled: Boolean(row.enabled),
    matchAll: Boolean(row.match_all),
...
function getMailRule(id) {
  const row = db.prepare('SELECT * FROM mail_rules WHERE id=?').get(Number(id));
  return row ? listMailRules(row.account_id).find(rule => rule.id === row.id) : null;
}

function saveMailRule(input = {}) {
  const now = Date.now();
  const accountId = String(input.accountId || '').trim();
  if (!accountId) throw new Error('Compte obligatoire pour une règle de messagerie');
  const conditions = Array.isArray(input.conditions) ? input.conditions.slice(0, 10).map(condition => ({
    field: ['from', 'to', 'subject', 'body'].includes(condition.field) ? condition.field : 'subject',
    op: ['contains', 'notContains', 'equals', 'startsWith'].includes(condition.op) ? condition.op : 'contains',
    value: String(condition.value || '').trim().slice(0, 300),
  })).filter(condition => condition.value) : [];
  if (!conditions.length) throw new Error('Au moins une condition est requise');
  const actions = input.actions && typeof input.actions === 'object' ? input.actions : {};

  if (input.id) {
    const existing = db.prepare('SELECT id FROM mail_rules WHERE id=?').get(Number(input.id));
    if (!existing) throw new Error('Règle introuvable');
    db.prepare(`UPDATE mail_rules SET account_id=@accountId, name=@name, enabled=@enabled,
                  match_all=@matchAll, stop_processing=@stopProcessing,
                  conditions_json=@conditionsJson, actions_json=@actionsJson, updated_at=@now
                WHERE id=@id`).run({
      id: Number(input.id), accountId, name: String(input.name || '').trim().slice(0, 120),
      enabled: input.enabled === false ? 0 : 1,
      matchAll: input.matchAll === false ? 0 : 1,
      stopProcessing: input.stopProcessing ? 1 : 0,
      conditionsJson: JSON.stringify(conditions), actionsJson: JSON.stringify(actions), now,
    });
    return getMailRule(input.id);
  }

  const position = db.prepare('SELECT COALESCE(MAX(position), -1) + 1 AS next FROM mail_rules WHERE account_id=?')
    .get(accountId).next;
  const result = db.prepare(`INSERT INTO mail_rules
      (account_id, name, enabled, match_all, stop_processing, conditions_json, actions_json, position, created_at, updated_at)
      VALUES (@accountId, @name, @enabled, @matchAll, @stopProcessing, @conditionsJson, @actionsJson, @position, @now, @now)`).run({
    accountId, name: String(input.name || '').trim().slice(0, 120),
    enabled: input.enabled === false ? 0 : 1,
    matchAll: input.matchAll === false ? 0 : 1,
    stopProcessing: input.stopProcessing ? 1 : 0,
    conditionsJson: JSON.stringify(conditions), actionsJson: JSON.stringify(actions), position, now,
  });
  return getMailRule(result.lastInsertRowid);
}

function removeMailRule(id) {
  return db.prepare('DELETE FROM mail_rules WHERE id=?').run(Number(id)).changes > 0;
}

function reorderMailRules(accountId, orderedIds) {
  const ids = (Array.isArray(orderedIds) ? orderedIds : []).map(Number).filter(Number.isInteger);
  const update = db.prepare('UPDATE mail_rules SET position=? WHERE id=? AND account_id=?');
  const transaction = db.transaction(() => {
    ids.forEach((id, index) => update.run(index, id, String(accountId)));
  });
  transaction();
  return listMailRules(accountId);
}
...
// ---------- Abonnements de contacts (vCard en lecture seule) ----------

function contactSubscriptionRow(row) {
  if (!row) return null;
  return {
    id: Number(row.id),
    name: row.name,
    url: row.url,
    enabled: Boolean(row.enabled),
    refreshMinutes: Number(row.refresh_minutes) || 0,
    etag: row.etag,
    lastModified: row.last_modified,
    lastSyncAt: Number(row.last_sync_at) || 0,
    lastStatus: row.last_status,
...
function normalizeContactRefreshMinutes(value, fallback = 30) {
  const numeric = Math.round(Number(value));
  if (!Number.isFinite(numeric)) return fallback;
  if (numeric <= 0) return 0;
  return Math.max(5, Math.min(1440, numeric));
...
function listContactSubscriptions({ enabledOnly = false } = {}) {
  const where = enabledOnly ? 'WHERE enabled=1' : '';
  return db.prepare(`SELECT * FROM contact_subscriptions ${where} ORDER BY name COLLATE NOCASE, id`).all().map(contactSubscriptionRow);
}

function getContactSubscription(id) {
  return contactSubscriptionRow(db.prepare('SELECT * FROM contact_subscriptions WHERE id=?').get(Number(id)));
}

function saveContactSubscription(input = {}, id = null) {
  const name = String(input.name || '').trim().slice(0, 240);
  const url = String(input.url || '').trim().slice(0, 4096);
  if (!url) throw new Error('URL du carnet d’adresses obligatoire');
  const enabled = input.enabled === false ? 0 : 1;
  const refreshMinutes = normalizeContactRefreshMinutes(input.refreshMinutes, 30);
  const now = Date.now();
  const numericId = Number(id || input.id || 0);
  if (numericId > 0) {
    const result = db.prepare(`UPDATE contact_subscriptions SET name=?, url=?, enabled=?, refresh_minutes=?, updated_at=? WHERE id=?`)
      .run(name, url, enabled, refreshMinutes, now, numericId);
    if (!result.changes) throw new Error('Abonnement introuvable');
    return getContactSubscription(numericId);
  }
  const result = db.prepare(`INSERT INTO contact_subscriptions
    (name,url,enabled,refresh_minutes,etag,last_modified,last_sync_at,last_status,last_error,created_at,updated_at)
    VALUES (?,?,?,?,'','',0,'','',?,?)`).run(name, url, enabled, refreshMinutes, now, now);
  return getContactSubscription(result.lastInsertRowid);
}

function updateContactSubscriptionSync(id, patch = {}) {
  const current = getContactSubscription(id);
  if (!current) throw new Error('Abonnement introuvable');
  const values = {
    etag: patch.etag === undefined ? current.etag : String(patch.etag || '').slice(0, 1000),
    lastModified: patch.lastModified === undefined ? current.lastModified : String(patch.lastModified || '').slice(0, 1000),
  ...
  db.prepare(`UPDATE contact_subscriptions SET etag=?,last_modified=?,last_sync_at=?,last_status=?,last_error=?,updated_at=? WHERE id=?`).run(
    values.etag, values.lastModified, values.lastSyncAt, values.lastStatus, values.lastError, Date.now(), Number(id),
  );
  return getContactSubscription(id);
}

function removeContactSubscription(id) {
  const subId = Number(id);
  const transaction = db.transaction(() => {
    const contacts = db.prepare('DELETE FROM contacts WHERE subscription_id=?').run(subId).changes;
    const subscription = db.prepare('DELETE FROM contact_subscriptions WHERE id=?').run(subId).changes;
    return { removed: subscription > 0, removedContacts: contacts };
  });
  return transaction();
}

/**
 * Les adresses email sont uniques dans tout le carnet d'adresses (contrainte
 * SQLite). Une adresse déjà utilisée par un contact créé manuellement ou
 * provenant d'un autre abonnement est donc ignorée plutôt que de faire
 * échouer l'ensemble de la synchronisation.
 */
function syncContactSubscriptionEntries(subscriptionId, entries = []) {
  const subId = Number(subscriptionId);
  if (!(subId > 0)) throw new Error('Abonnement invalide');
  const existingRows = db.prepare('SELECT id, import_key FROM contacts WHERE subscription_id=?').all(subId);
  const existingByKey = new Map(existingRows.filter(row => row.import_key).map(row => [row.import_key, row]));
  const now = Date.now();
  let created = 0, updated = 0, skipped = 0, removed = 0, skippedEmails = 0;
  const incomingKeys = new Set();

  const insertContact = db.prepare(`INSERT INTO contacts(display_name,first_name,last_name,company,
    job_title,phone,mobile,postal_address,birthday,notes,avatar_data,trusted,favorite,
    preferred_account_id,import_key,subscription_id,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
  const updateContact = db.prepare(`UPDATE contacts SET display_name=?, first_name=?, last_name=?, company=?,
    job_title=?, phone=?, mobile=?, postal_address=?, birthday=?, notes=?, updated_at=? WHERE id=?`);
  const deleteEmails = db.prepare('DELETE FROM contact_emails WHERE contact_id=?');
  const insertEmail = db.prepare('INSERT INTO contact_emails(contact_id,email,label,is_primary) VALUES(?,?,?,?)');
  const deleteContact = db.prepare('DELETE FROM contacts WHERE id=?');
  const emailOwner = db.prepare('SELECT contact_id FROM contact_emails WHERE email=?');

  const transaction = db.transaction(items => {
    for (const entry of items) {
      const displayName = String(entry?.displayName || '').trim().slice(0, 240);
      const importKey = String(entry?.importKey || '').trim();
      if (!importKey || !displayName) { skipped += 1; continue; }
      incomingKeys.add(importKey);
      const emails = (Array.isArray(entry.emails) ? entry.emails : [])
        .map(item => ({ email: String(item?.email || '').trim().toLowerCase(), label: String(item?.label || '').trim().slice(0, 60) }))
        .filter(item => /.+@.+\..+/.test(item.email));
      ...
      if (existing) {
        ...
        updateContact.run(
          displayName, String(entry.firstName || '').slice(0, 120), String(entry.lastName || '').slice(0, 120),
          String(entry.company || '').slice(0, 200), String(entry.jobTitle || '').slice(0, 200),
          String(entry.phone || '').slice(0, 60), String(entry.mobile || '').slice(0, 60),
          String(entry.postalAddress || '').slice(0, 500), String(entry.birthday || '').slice(0, 20),
          String(entry.notes || '').slice(0, 4000), now, contactId,
        );
        deleteEmails.run(contactId);
        updated += 1;
      } else {
        const result = insertContact.run(
          ...
          String(entry.phone || '').slice(0, 60), String(entry.mobile || '').slice(0, 60),
          String(entry.postalAddress || '').slice(0, 500), String(entry.birthday || '').slice(0, 20),
          String(entry.notes || '').slice(0, 4000), '', 1, 0, null, importKey, subId, now, now,
        );
        contactId = Number(result.lastInsertRowid);
        created += 1;
      }
      ...
      emails.forEach((item, index) => {
        const owner = emailOwner.get(item.email);
        if (owner && Number(owner.contact_id) !== contactId) { skippedEmails += 1; return; }
        try { insertEmail.run(contactId, item.email, item.label, index === 0 ? 1 : 0); anyEmail = true; }
        catch { skippedEmails += 1; }
      });
      if (!anyEmail && !existing) {
        // Aucune adresse exploitable : un contact d'abonnement sans email n'est pas conservé.
        deleteContact.run(contactId);
        created -= 1;
        incomingKeys.delete(importKey);
      }
    }
    for (const row of existingRows) {
      if (!row.import_key || incomingKeys.has(row.import_key)) continue;
      removed += deleteContact.run(row.id).changes;
    }
  });
  transaction(Array.isArray(entries) ? entries : []);
  return { created, updated, unchanged: 0, skipped, removed, skippedEmails, total: created + updated };
}
...
module.exports = {
  ...
  saveContactSubscription,
  updateContactSubscriptionSync,
  removeContactSubscription,
  syncContactSubscriptionEntries,
};
