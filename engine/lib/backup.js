/**
 * FARO Mail — Sauvegarde/restauration complète au format ZIP.
 *
...
const FORMAT = 'faromail-backup';
// Anciennes sauvegardes créées avant le renommage LibraMail -> FARO Mail :
// acceptées en lecture pour ne pas casser une restauration existante.
const LEGACY_FORMAT = 'libramail-backup';
...
async function readEntryBuffer(zipPath, entry, maxBytes = 8 * 1024 * 1024) {
  ...
  const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'faromail-entry-'));
...
async function inspectArchive(sourcePath) {
  ...
  if (!manifestEntry) throw new Error('Cette archive ne contient pas de manifeste FARO Mail');
  let manifest;
  try {
  ...
  if (![FORMAT, LEGACY_FORMAT].includes(manifest?.format) || ![1, FORMAT_VERSION].includes(formatVersion)) {
    throw new Error('Cette archive n’est pas une sauvegarde FARO Mail compatible');
  }
...
async function exportArchive({ dataDir, database, targetPath, appVersion, password = '', onProgress }) {
  ...
  if (target === resolvedData || target.startsWith(resolvedData + path.sep)) {
    throw new Error('La sauvegarde doit être enregistrée en dehors du dossier data de FARO Mail');
  }
  await fsp.mkdir(path.dirname(target), { recursive: true });

  const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'faromail-backup-'));
