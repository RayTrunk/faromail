/**
 * FARO Mail — Import de calendriers (iCalendar/vCalendar/CSV)
 *
 * Le parseur reste volontairement sans dépendance externe afin que les paquets
 * portables FARO Mail conservent leur caractère autonome.
 */
'use strict';
...
function normalizeIcsEvent(rawEvent, calendar, sourceName, accountId, fallbackColor) {
  ...
  // Une couleur choisie dans FARO Mail est prioritaire sur la couleur embarquée
...
function parseCsv(text, { fileName = '', accountId = '', color = '', locale = 'de', importNamespace = '' } = {}) {
  ...
  const preferDmy = normalizedLocale.startsWith('fr') || normalizedLocale.startsWith('de');
  const events = [];
...
function parseCalendarImport({ text = '', fileName = '', accountId = '', color = '', locale = 'de', importNamespace = '' } = {}) {
