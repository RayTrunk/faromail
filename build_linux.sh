#!/usr/bin/env bash
# ============================================================================
# FARO Mail — Construction d'un paquet GNU/Linux autonome
#
...
KEEP_WORK=0
OFFLINE=0
NODE_VERSION="${FAROMAIL_NODE_VERSION:-24.18.0}"

usage() {
  cat <<'USAGE'
FARO Mail — construction Linux autonome
...
Variables facultatives :
  FAROMAIL_NODE_VERSION  Même rôle que --node-version.
  FAROMAIL_NODE_MIRROR   Miroir Node.js, par défaut https://nodejs.org/dist
USAGE
}
...
if pgrep -f "${PROJECT_DIR//\//\\/}/engine/backend\.js" >/dev/null 2>&1; then
  die "Le moteur FARO Mail semble actif. Fermez l'application avant la compilation."
fi
...
print(cfg.get('cli', {}).get('binaryName', 'faromail'))
print(cfg.get('applicationId', 'eu.faromail.app'))
PY
)
...
NODE_MIRROR="${FAROMAIL_NODE_MIRROR:-https://nodejs.org/dist}"
NODE_ARCHIVE="node-v${NODE_VERSION}-linux-${NODE_ARCH}.tar.xz"
NODE_FOLDER="node-v${NODE_VERSION}-linux-${NODE_ARCH}"
NODE_RELEASE_URL="${NODE_MIRROR%/}/v${NODE_VERSION}"

CACHE_DIR="$PROJECT_DIR/.cache/faromail-node/v${NODE_VERSION}"
...
PACKAGE_NAME="FaroMail-${VERSION}-linux-${PACKAGE_ARCH}"
...
cfg.setdefault('modes', {}).setdefault('window', {})['title'] = f'FARO Mail {version}'
...
install -m 0755 "$NEU_BINARY" "$PACKAGE_DIR/faromail-app"
...
# ---------------------------------------------------------------------------
# Moteur FARO Mail et dépendances npm
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
cat > "$PACKAGE_DIR/faromail" <<'LAUNCHER'
#!/usr/bin/env bash
set -Eeuo pipefail
IFS=$'\n\t'
...
[[ -f "$ENGINE_FILE" ]] || {
  printf 'Erreur : moteur FARO Mail introuvable : %s\n' "$ENGINE_FILE" >&2
  exit 1
}
[[ -x "$APP_BIN" ]] || {
...
if port_is_open; then
  printf 'Erreur : le port 47800 est déjà utilisé. Une autre instance de FARO Mail est probablement ouverte.\n' >&2
  exit 1
fi
...
for _ in $(seq 1 150); do
  ...
  if ! kill -0 "$ENGINE_PID" >/dev/null 2>&1; then
    printf 'Le moteur FARO Mail ne démarre pas. Consultez : %s\n' "$LOG_FILE" >&2
...
"$APP_BIN" "$@"
LAUNCHER
chmod 0755 "$PACKAGE_DIR/faromail"
...
FARO Mail ${VERSION} — paquet autonome GNU/Linux ${PACKAGE_ARCH}
...
Le dossier FARO Mail peut être déplacé en bloc. Ne déplacez pas uniquement le
fichier faromail : il dépend de faromail-app, runtime/, engine/ et data/.
...
[[ -x "$PACKAGE_DIR/faromail" ]] || die "Lanceur absent."
[[ -x "$PACKAGE_DIR/faromail-app" ]] || die "Binaire Neutralino absent."
[[ -x "$PACKAGE_NODE" ]] || die "Runtime Node.js absent."
[[ -f "$PACKAGE_DIR/engine/backend.js" ]] || die "Moteur absent."

bash -n "$PACKAGE_DIR/faromail"
bash -n "$PACKAGE_DIR/check_portable.sh"

grep -Fq 'runtime/node/bin/node' "$PACKAGE_DIR/faromail" \
  || die "Le lanceur ne référence pas le runtime embarqué."
if grep -Eq '(^|[[:space:]])node[[:space:]]+.*backend\.js' "$PACKAGE_DIR/faromail"; then
