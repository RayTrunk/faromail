...
if port_is_open; then
  echo "FARO Mail est déjà en cours d'exécution (port 47800 occupé)." >&2
  exit 1
fi
...
for _ in $(seq 1 100); do
  ...
  if ! kill -0 "$ENGINE_PID" >/dev/null 2>&1; then
    ...
    echo "Le moteur FARO Mail n'a pas pu démarrer." >&2
    exit 1
  fi
  sleep 0.1
done
...
if ! port_is_open; then
  echo "Délai dépassé pendant le démarrage du moteur FARO Mail." >&2
  exit 1
fi

if [ -x "./bin/faromail-linux_x64" ]; then
  ./bin/faromail-linux_x64 --load-dir-res --path=. --neu-dev-extension
else
