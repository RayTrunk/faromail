# Publishing FARO Mail with GitHub Actions

> **English** · [Français](#publication-de-faro-mail-avec-github-actions)
...
Copy this kit to the root of the FARO Mail source tree, then run:
...
```bash
GITHUB_OWNER=my-account REPO_NAME=faromail VISIBILITY=private ./github.sh init
```
...
# Publication de FARO Mail avec GitHub Actions
...
Copiez ce kit à la racine des sources FARO Mail puis lancez :
...
```bash
GITHUB_OWNER=mon-compte REPO_NAME=faromail VISIBILITY=private ./github.sh init
```
