...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'faromail-mailstore-test-'));
(async () => {
try {
  db.init(temp);
  mailStore.init(temp);
  const raw = Buffer.from([
    ...
    '',
    'Contenu confidentiel FARO Mail.',
  ].join('\r\n'));
  const row = {
    ...
    date: Date.now(), snippet: 'Contenu confidentiel FARO Mail.', seen: 0,
  ...
  console.log('[FARO Mail] Test magasin chiffré : OK');
} finally {
