...
<#
FARO Mail - construction d'un paquet Windows x64 autonome à lancement natif.
...
$NodeMirror = if ($env:FAROMAIL_NODE_MIRROR) {
    $env:FAROMAIL_NODE_MIRROR.TrimEnd('/')
}
else {
...
$CacheDir = Join-Path $ProjectDir ".cache\faromail-node\v$NodeVersion"
...
$PackageName = "FaroMail-$Version-windows-x86_64"
...
try {
    ...
    # sont injectées dans FaroMail.exe pour avoir un seul exécutable à lancer.
    ...
    Copy-Item `
        ...
        -Destination (Join-Path $PackageDir 'FaroMail.exe') `
        -Force

    Write-Ok -Message 'Ressources Neutralino intégrées directement dans FaroMail.exe.'
    ...
    # Les shims npm (.cmd/.ps1) ne servent pas à l'exécution de FARO Mail.
    ...
    $appExePath = Join-Path $PackageDir 'FaroMail.exe'
    if (-not (Test-Path -LiteralPath $appExePath -PathType Leaf)) {
        throw 'FaroMail.exe est absent du paquet Windows.'
    }
    if (Test-Path -LiteralPath (Join-Path $PackageDir 'resources.neu')) {
        throw 'resources.neu ne doit pas être présent : les ressources doivent être intégrées dans FaroMail.exe.'
    }
    ...
    $readmeText = $readmeText.Replace('__FAROMAIL_VERSION__', $Version)
